 ...
 * Pages: Overview | View Events | Register Event | My Events | Feedback | My Attendance | Certificates
 *
 * Key differences from Student:
 * - No club membership, no join/leave club
 * - Registers for staff_event (institution-level events) only
 * - Mobile number collected at event registration (staff_event_registration)
 * - Attendance drawn from student_attendance linked to staff_event
 * - Certificates drawn from certificate linked to staff_event
 */
public class ExternalGuestDashboard extends JFrame {
    ...
    // Pages: 0=Overview 1=ViewEvents 2=RegisterEvent 3=MyEvents 4=Feedback 5=MyAttendance 6=Certificates
    ...
    // Models
    private DefaultTableModel viewEventsModel;
        ...
        rootLayer = new JPanel(null) {
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.setPaint(new RadialGradientPaint(W/2f,H*0.3f,W*0.5f,
                    new float[]{0f,1f}, new Color[]{new Color(100,40,220,40),new Color(0,0,0,0)}));
                g2.fillRect(0,0,W,H);
            }
        };
    ...
    private void startAutoRefresh() {
        autoRefreshTimer = new javax.swing.Timer(15000, e -> {
            if (curPage == 5) refreshMyAttendancePage();
            if (curPage == 6) refreshCertificatesPage();
        });
    ...
    private void buildTopBar(Dimension scr) {
        ...
        greet.setBounds(pw-390,(TOP_H-18)/2,260,18); top.add(greet);
        JButton logout = buildButton("Logout", false);
        logout.setBounds(pw-126,(TOP_H-34)/2,110,34);
        logout.addActionListener(e -> {
            if (autoRefreshTimer != null) autoRefreshTimer.stop();
            dispose(); SwingUtilities.invokeLater(WelcomePage::new);
        });
        top.add(logout);
        rootLayer.add(top);
    }
    ...
    private void navigateTo(int page) {
        ...
        slideTimer = new Timer(12, e -> {
            ...
            rootLayer.repaint();
            if (slideOff >= 1f) {
                ((Timer)e.getSource()).stop(); sliding = false; curPage = tgtPage;
        ...
        new Thread(() -> {
            try (Connection con = DriverManager.getConnection(dbUrl,dbUser,dbPass)) {
                ...
                if (rs.next()) {
                    ...
                    orgName      = rs.getString("organisation_name");
                    roleType     = rs.getString("role_type");
                    java.sql.Date dob = rs.getDate("dob");
                    if (dob != null) {
                        guestDOB = dob.toString();
    ...
    private void buildOverviewPage(Dimension scr) {
        JPanel pg = pages[0]; int W = pw-SIDEBAR_W_PX, H = ph-TOP_H, cx = W/2;
        ...
        refreshBtn.setBounds(W-180,20,160,36); refreshBtn.setFont(new Font("Georgia",Font.BOLD,12));
        ...
        ttl.setHorizontalAlignment(SwingConstants.CENTER); ttl.setBounds(0,24,W-200,38); content.add(ttl);
        ...
        // Stat cards
        int scX=pcX+pcW+28, scY=pcY, scW=(W-scX-32)/3, scH=120, scGap=14;
        String[][] stats = {
            {"\uD83D\uDCC5","Events Registered",
        ...
        String html = "<html><body style='color:rgb(160,150,200);font-family:SansSerif;font-size:12px;line-height:1.7'>" +
            ...
            "<b style='color:rgb(240,235,255)'>Event Registration:</b> Browse and register for institution-level events " +
            "(College Day, Symposium, Hackathon, etc.) organised by staff. Your mobile number is recorded at registration.<br><br>" +
            "<b style='color:rgb(240,235,255)'>Attendance &amp; Certificates:</b> Your attendance is marked by staff after events. " +
            "Certificates issued to you appear in the Certificates page — you can view, download or print them.<br><br>" +
    ...
    // ══════════════════════════════════════════════════════════════
    //  PAGE 1 — VIEW EVENTS (only upcoming staff_event entries)
    // ══════════════════════════════════════════════════════════════
    private void buildViewEventsPage(Dimension scr) {
        JPanel pg = pages[1]; int W = pw-SIDEBAR_W_PX, H = ph-TOP_H;
        pageTitle(pg, "View Events", W);

        JPanel banner = glassCard(); banner.setBounds(24,66,W-48,44); pg.add(banner);
        JLabel bl = makeLabel("Upcoming institution-level events open to external guests.",
        ...
        viewEventsModel = noEditModel(cols);
        JTable table = new JTable(viewEventsModel); styleTable(table);

        JButton refreshBtn = buildButton("\u21BA Refresh", true);
        refreshBtn.setBounds(W-180,20,160,36); refreshBtn.setFont(new Font("Georgia",Font.BOLD,12));
        refreshBtn.addActionListener(e -> { viewEventsModel.setRowCount(0); loadViewEventsData(); });
        pg.add(refreshBtn);
    ...
    private void loadViewEventsData() {
        new Thread(() -> {
            try (Connection con = DriverManager.getConnection(dbUrl,dbUser,dbPass)) {
                PreparedStatement ps = con.prepareStatement(
                    ...
                    "WHERE se.start_date >= TRUNC(SYSDATE) " +
                ...
                while (rs.next()) {
                    ...
                    SwingUtilities.invokeLater(() -> viewEventsModel.addRow(row));
                }
            } catch (SQLException ex) {
                SwingUtilities.invokeLater(() -> viewEventsModel.addRow(
                    new Object[]{"—","Error: "+ex.getMessage().substring(0,Math.min(40,ex.getMessage().length())),
                        "—","—","—","—","—","—","—"}));
            }
        }).start();
    }
    ...
    // ══════════════════════════════════════════════════════════════
    //  PAGE 2 — REGISTER EVENT (full validations matching student)
    // ══════════════════════════════════════════════════════════════
    private void buildRegisterEventPage(Dimension scr) {
        JPanel pg = pages[2]; int W = pw-SIDEBAR_W_PX, H = ph-TOP_H, cx = W/2;
        pageTitle(pg, "Register for Event", W);

        int cw = 520, ch = 440, cardX = cx-cw/2, cardY = 76;
        ...
        detLbl.setBounds(fx,fy,fw,80); detLbl.setVerticalAlignment(SwingConstants.TOP); card.add(detLbl); fy += 90;
        ...
        evCombo.addActionListener(e -> {
            ...
            if (parts.length >= 4)
                detLbl.setText("<html><b style='color:rgb(240,235,255)'>"
                    +parts[1].trim()+"</b><br>"
                    +"<span style='color:rgb(160,150,200)'>Type: "+parts[2].trim()
                    +"&nbsp;&nbsp;Date: "+parts[3].trim()+"</span></html>");
        });
        ...
        // Load combo (excluding already-registered events)
        ...
        regBtn.addActionListener(e -> {
            ...
            if (mobile.isEmpty() || mobile.length() != 10 || !mobile.matches("\\d{10}")) {
                ...
                statusLbl.setText("Enter a valid 10-digit mobile number."); return;
            }
            // Validation 2: event selected
            if (sel == null || sel.startsWith("--")) {
                statusLbl.setForeground(ERROR_COL);
                statusLbl.setText("Please select an event."); return;
            }
            int evId = Integer.parseInt(sel.split("\\|")[0].trim());

            // Validation 3: event date not passed (checked in DB thread)
            registerGuestForEvent(evId, mobile, statusLbl, evCombo, mobileField);
        });
    }
    ...
    private void buildMyEventsPage(Dimension scr) {
        ...
        table.getColumn("Status").setCellRenderer(new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(
                ...
                Date endDate = (Date) tbl.getValueAt(row, 5);
                String status = (String) v;
                if ("Registered".equalsIgnoreCase(status) && endDate != null && endDate.before(new Date())) {
        ...
        JButton refreshBtn = buildButton("\u21BA Refresh", true);
        refreshBtn.setBounds(W-180,20,160,36); refreshBtn.setFont(new Font("Georgia",Font.BOLD,12));
        refreshBtn.addActionListener(e -> loadMyEventsData());
        pg.add(refreshBtn);
        ...
        new Thread(() -> {
            try (Connection con = DriverManager.getConnection(dbUrl,dbUser,dbPass)) {
                ...
                while (rs.next()) {
                    ...
                    SwingUtilities.invokeLater(() -> myEventsModel.addRow(row));
                }
            } catch (SQLException ex) {
                SwingUtilities.invokeLater(() -> myEventsModel.addRow(
                    new Object[]{"—","Error","—","—","—","—","—","—","—"}));
            }
        }).start();
    }
    ...
    // ══════════════════════════════════════════════════════════════
    //  PAGE 4 — FEEDBACK (only for completed events)
    // ══════════════════════════════════════════════════════════════
    private void buildFeedbackPage(Dimension scr) {
        JPanel pg = pages[4]; int W = pw-SIDEBAR_W_PX, H = ph-TOP_H, cx = W/2;
        pageTitle(pg, "Give Feedback", W);

        String[] cols = {"Feedback ID","Event Title","Type","Rating","Comments"};
        feedbackHistModel = noEditModel(cols);
        ...
        refreshBtn.addActionListener(e -> {
            feedbackHistModel.setRowCount(0); loadFeedbackHistory(feedbackHistModel);
            if (feedbackEvCombo != null) loadGuestFeedbackEventCombo(feedbackEvCombo);
        });
        pg.add(refreshBtn);
        ...
        JLabel noteLabel = makeLabel("(Only completed events are available)", new Font("SansSerif",Font.ITALIC,11), WARN_COL);
        noteLabel.setBounds(fx,cy,300,16); card.add(noteLabel); cy += 22;

        card.add(fldLblC("Select Event",  fx, cy, fw-120));
        card.add(fldLblC("Rating", fx+fw-110, cy, 110)); cy += 18;
        feedbackEvCombo = styledCombo(new String[]{"-- Loading --"});
        ...
        loadGuestFeedbackEventCombo(feedbackEvCombo);
    ...
    // ══════════════════════════════════════════════════════════════
    //  PAGE 5 — MY ATTENDANCE
    // ══════════════════════════════════════════════════════════════
    private void buildMyAttendancePage(Dimension scr) {
        JPanel pg = pages[5]; int W = pw-SIDEBAR_W_PX, H = ph-TOP_H;
        pg.removeAll();
        ...
        JPanel banner = glassCard(); banner.setBounds(24,cardY+cardH+10,W-48,36); pg.add(banner);
        JLabel bannerLbl = makeLabel(
            "\u2139  Attendance is marked by staff after the event. This page auto-refreshes every 15 seconds.",
        ...
        myAttendanceModel = noEditModel(cols);
        ...
        table.getColumn("Status").setCellRenderer(new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(
                ...
                if ("Present".equalsIgnoreCase(s)) {
                    ...
                    lbl.setForeground(WARN_COL); lbl.setText("\u2014  "+s);
                }
        ...
        pg.add(spAt(styledTableScroll(table), 24, tableY, W-48, H-tableY-24));

        loadMyAttendanceData(myAttendanceModel, myAttTotalCard, myAttPresentCard, myAttAbsentCard);

        refreshBtn.addActionListener(e -> {
            myAttendanceModel.setRowCount(0);
            setStatCardValue(myAttTotalCard, "—"); setStatCardValue(myAttPresentCard, "—"); setStatCardValue(myAttAbsentCard, "—");
            loadMyAttendanceData(myAttendanceModel, myAttTotalCard, myAttPresentCard, myAttAbsentCard);
        });
    }

    private void loadMyAttendanceData(DefaultTableModel model,
                                      JPanel totalCard, JPanel presentCard, JPanel absentCard) {
        new Thread(() -> {
            try (Connection con = DriverManager.getConnection(dbUrl,dbUser,dbPass)) {
                // Guest attendance is linked only to staff_event
                PreparedStatement ps = con.prepareStatement(
    ...
    private void buildCertificatesPage(Dimension scr) {
        JPanel pg = pages[6]; int W = pw-SIDEBAR_W_PX, H = ph-TOP_H;
        pg.removeAll();
        pageTitle(pg, "My Certificates", W);

        certTotalCard = statCard("\uD83C\uDFC5","Total Certificates","—");
        certTotalCard.setBounds(24,72,210,90); pg.add(certTotalCard);

        JButton refreshBtn = buildButton("\u21BA Refresh", true);
        refreshBtn.setBounds(W-180,20,160,36); refreshBtn.setFont(new Font("Georgia",Font.BOLD,12));
        pg.add(refreshBtn);

        JPanel banner = glassCard(); banner.setBounds(24,172,W-48,38); pg.add(banner);
        JLabel bannerLbl = makeLabel(
            "\u2139  Certificates are issued by staff after events. Click \"\uD83C\uDF93 View\" to open, download PNG, or print/save as PDF.",
            new Font("SansSerif",Font.PLAIN,12), TEXT_MUTED);
        bannerLbl.setBounds(12,10,W-96,18); banner.add(bannerLbl);

        String[] cols = {"Cert ID","Event Name","Certificate Type","Issue Date","Action"};
            ...
            @Override public Component getTableCellEditorComponent(
                ...
                btn.addActionListener(e -> {
                    ...
                    if (!(val instanceof Integer)) {
                        JOptionPane.showMessageDialog(null, "Invalid Certificate"); return;
                    }
                    int certId = (Integer) val;
                    String evName    = (String) tbl.getValueAt(row, 1);
                    String certType  = (String) tbl.getValueAt(row, 2);
                    Object issueDateObj = tbl.getValueAt(row, 3);
        ...
        table.getColumn("Event Name")       .setPreferredWidth(340);
        table.getColumn("Certificate Type") .setPreferredWidth(160);
        table.getColumn("Issue Date")       .setPreferredWidth(120);
        ...
        refreshBtn.addActionListener(e -> {
            certificatesModel.setRowCount(0); setStatCardValue(certTotalCard, "—");
            loadCertificatesData(certificatesModel);
        });
    }
    ...
    private void loadCertificatesData(DefaultTableModel model) {
        new Thread(() -> {
            try (Connection con = DriverManager.getConnection(dbUrl,dbUser,dbPass)) {
                // Guests receive certs linked to staff_event only
                PreparedStatement ps = con.prepareStatement(
                    "SELECT cert.certificate_id, se.event_title, " +
                ...
                while (rs.next()) {
                    count++;
                    Object[] row = {rs.getInt(1),rs.getString(2),rs.getString(3),rs.getDate(4),"\uD83C\uDF93 View"};
                    SwingUtilities.invokeLater(() -> model.addRow(row));
                }
                final int finalCount = count;
                SwingUtilities.invokeLater(() -> setStatCardValue(certTotalCard, String.valueOf(finalCount)));
            } catch (SQLException ex) {
                SwingUtilities.invokeLater(() -> {
                    model.addRow(new Object[]{"—","Error: "+ex.getMessage(),"—","—","—"});
    ...
    private void openCertificateViewer(int certId, String evName, String certType, String issueDate) {
        new Thread(() -> {
            CertificateViewer.CertData cd = new CertificateViewer.CertData();
            cd.certId      = certId;
            cd.studentName = guestName;
            cd.eventName   = evName;
            cd.certType    = certType;
            cd.issueDate   = issueDate;
            cd.deptName    = "";
            SwingUtilities.invokeLater(() -> CertificateViewer.show(ExternalGuestDashboard.this, cd));
        }).start();
    }
    ...
    /** Register guest — with full validations: duplicate, event date, mobile */
    private void registerGuestForEvent(int eventId, String mobile, JLabel statusLbl,
                                        JComboBox<String> evCombo, JTextField mobileField) {
        new Thread(() -> {
            try (Connection con = DriverManager.getConnection(dbUrl,dbUser,dbPass)) {

                // Validation: event date not in the past
                PreparedStatement datePsCheck = con.prepareStatement(
                    "SELECT end_date FROM staff_event WHERE event_id=?");
                datePsCheck.setInt(1, eventId); ResultSet dr = datePsCheck.executeQuery();
                if (dr.next()) {
                    java.sql.Date endDate = dr.getDate(1);
                    if (endDate != null && endDate.before(new java.sql.Date(System.currentTimeMillis()))) {
                        SwingUtilities.invokeLater(() -> {
                            statusLbl.setForeground(ERROR_COL);
                            statusLbl.setText("This event has already passed.");
                        }); return;
                    }
                }

                // Validation: already registered
                PreparedStatement dupPs = con.prepareStatement(
                    "SELECT COUNT(*) FROM staff_event_registration WHERE event_id=? AND user_id=?");
                dupPs.setInt(1, eventId); dupPs.setInt(2, userId);
                ResultSet dup = dupPs.executeQuery();
                if (dup.next() && dup.getInt(1) > 0) {
                    SwingUtilities.invokeLater(() -> {
                        statusLbl.setForeground(ERROR_COL);
                        statusLbl.setText("You are already registered for this event.");
                    }); return;
                }
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    mobileField.setText("");
                    // Reload combo to remove the registered event
                    loadAllStaffEventsCombo(evCombo);
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    if (msg.contains("ORA-00001"))
                        statusLbl.setText("Already registered for this event.");
                    else
    ...
    private void loadAllStaffEventsCombo(JComboBox<String> combo) {
        new Thread(() -> {
            try (Connection con = DriverManager.getConnection(dbUrl, dbUser, dbPass)) {
                ...
                CallableStatement cs = con.prepareCall("{call Get_Available_Events(?, ?)}");
                ...
                try (ResultSet rs = (ResultSet) cs.getObject(2)) {
                    ...
                    while (rs.next()) {
                        String display = rs.getInt("event_id") + " | " 
    ...
    private void loadFeedbackHistory(DefaultTableModel model) {
        new Thread(() -> {
            try (Connection con = DriverManager.getConnection(dbUrl,dbUser,dbPass)) {
                ...
                ps.setInt(1, userId); ResultSet rs = ps.executeQuery();
                while (rs.next()) {
                    Object[] row = {rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4),rs.getString(5)};
                    SwingUtilities.invokeLater(() -> model.addRow(row));
                }
            } catch (SQLException ex) {
                SwingUtilities.invokeLater(() -> model.addRow(new Object[]{"—","Error","—","—","—"}));
            }
        }).start();
    }
        ...
        new Thread(() -> {
            try (Connection con = DriverManager.getConnection(dbUrl,dbUser,dbPass)) {
                ...
                PreparedStatement dup = con.prepareStatement(
                    "SELECT COUNT(*) FROM feedback WHERE user_id=? AND event_id=?");
                dup.setInt(1, userId); dup.setInt(2, eventId); ResultSet dr = dup.executeQuery();
                if (dr.next() && dr.getInt(1) > 0) {
                    SwingUtilities.invokeLater(() -> {
                        statusLbl.setForeground(ERROR_COL);
                        statusLbl.setText("You have already submitted feedback for this event.");
                    }); return;
                }
                ...
                ps.setInt(4,rating); ps.setString(5,cmt); ps.executeUpdate();

                histModel.setRowCount(0); loadFeedbackHistory(histModel);
                loadGuestFeedbackEventCombo(feedbackEvCombo);
                SwingUtilities.invokeLater(() -> {
                    statusLbl.setForeground(SUCCESS_COL); statusLbl.setText("Feedback submitted successfully!");
                    cmtArea.setText("");
                });
            } catch (SQLException ex) {
                SwingUtilities.invokeLater(() -> {
    ...
    private void pageTitle(JPanel pg, String text, int W) {
        ...
        t.setHorizontalAlignment(SwingConstants.LEFT); t.setBounds(24,18,W-220,36); pg.add(t);
